...
import { errorAdd, smurfsAdd } from '../actions'

const AddForm = ({errorMessage, errorAdd, smurfsAdd}) => {
    const initialState = {
        name:"",
        position:"",
        nickname:"",
    ...
    const handleSubmit = e => {
        ...
        if (state.name === "" || state.position === "" || state.nickname === "") {
            errorAdd("Name, position and nickname fields are required.")
        } else {
            smurfsAdd(state)
...
export default connect(mapStateToProps, { errorAdd, smurfsAdd })(AddForm);
